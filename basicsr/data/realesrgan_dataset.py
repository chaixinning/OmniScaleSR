...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'gt_path' in opt:
            if isinstance(opt['gt_path'], str):
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path']).rglob(f'*.{opt["image_type"]}')]))
            else:
                for path in opt['gt_path']:
                    self.paths.extend(sorted([str(x) for x in Path(path).rglob(f'*.{opt["image_type"]}')]))
