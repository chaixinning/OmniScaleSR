...
def uniform_scale_lq_pair(min_scale=4.0, max_scale=16.0, min_lq=32, max_hq=512):
    """
    均匀采样倍率 s, 并计算合法的 (LQ, HQ) 配对。
    """
    s = random.uniform(min_scale, max_scale)
    lq_max = int(math.floor(max_hq / s))
    lq = random.randint(min_lq, lq_max)
    lq = lq//8*8
    hq = int(round(s * lq))
    hq = hq//8*8
    return lq, hq, s

def custom_collate_fn_VariedSize(batch):
    gts = []
    conditioning_pixel_values = []
    ram_values = []
    input_ids = []

    lq_size, hq_size, s = uniform_scale_lq_pair()

    for item in batch:
        gt = item["pixel_values"]
        conditioning_pixel_value = item["conditioning_pixel_values"]
        ram_value = item["ram_values"]
        input_id = item["input_ids"]
        
        gt = F.interpolate(gt.unsqueeze(0), size=(hq_size, hq_size), mode='bicubic')
        conditioning_pixel_value = F.interpolate(conditioning_pixel_value.unsqueeze(0), size=(lq_size, lq_size), mode='bicubic')

        gts.append(gt.squeeze(0))
        conditioning_pixel_values.append(conditioning_pixel_value.squeeze(0))
        ram_values.append(ram_value)
        input_ids.append(input_id)

    batch_size = len(batch)
    batch_transform_res = {
        'pixel_values': torch.stack(gts),
        'conditioning_pixel_values': torch.stack(conditioning_pixel_values),
        'input_ids': torch.stack(input_ids),
        'ram_values': torch.stack(ram_values),
        'lq_size': torch.full((batch_size,), lq_size, dtype=torch.float),
        'sr_scale': torch.full((batch_size,), s, dtype=torch.float),
    }

    return batch_transform_res


def set_seed(seed):
    """设置随机种子以确保代码的可重复性。"""
    torch.manual_seed(seed)  # 为CPU设置随机种子
    torch.cuda.manual_seed(seed)  # 为所有CUDA设备设置随机种子
    torch.cuda.manual_seed_all(seed)  # 如果使用多个CUDA设备，为所有CUDA设备设置随机种子
    np.random.seed(seed)  # 为numpy设置随机种子
    random.seed(seed)  # 为Python的内置随机库设置随机种子
    torch.backends.cudnn.deterministic = True  # 确保CUDA的行为可重复
    torch.backends.cudnn.benchmark = False  # 如果网络输入数据维度或类型上变化不大，设置为True可能更高效
...
def load_sr4x_model(device='cuda'):
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--controlnet_unet_name_or_path",
        type=str,
        default=None,
        help="Path to pretrained controlnet unet model or model identifier from huggingface.co/models."
        " If not specified controlnet weights are initialized from unet.",
    )
    parser.add_argument(
        "--revision",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./experience/checkpoints/omniscalesr",
    ...
    parser.add_argument('--trainable_modules', nargs='*', type=str, default=["image_attentions", 
                                                                             "srscale_proj", "srscale_embedding", "srscale_proj_formod", "srscale_embedding_formod",
                                                                             "lqsize_proj", "lqsize_embedding", "lqsize_proj_formod", "lqsize_embedding_formod"])
...
if args.controlnet_unet_name_or_path:
    # resume from self-train
    logger.info("------------------------Loading unet weights from self-train--------------------------------------")
    unet = UNet2DConditionModel.from_pretrained(
        args.controlnet_unet_name_or_path, subfolder="unet", revision=args.revision, use_image_cross_attention=True, low_cpu_mem_usage=False,
    )
else:
    ...
    print(f'===== if use_image_cross_attention? {unet.config.use_image_cross_attention}')

if args.controlnet_model_name_or_path:
    # resume from self-train
    logger.info("Loading existing controlnet weights")
    controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path, subfolder="controlnet", low_cpu_mem_usage=False)
...
sr4x_model = load_sr4x_model(accelerator.device)
...
train_dataloader = torch.utils.data.DataLoader(
    ...
    collate_fn=custom_collate_fn_VariedSize,
...
sr4x_model.to(accelerator.device, dtype=weight_dtype)
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(unet):
            ...
            sr_scale = batch["sr_scale"].to(accelerator.device, dtype=weight_dtype)
            lq_size = batch["lq_size"].to(accelerator.device, dtype=weight_dtype)
            ...
            controlnet_image_in = batch["conditioning_pixel_values"].to(accelerator.device, dtype=weight_dtype)
            with torch.no_grad():
                controlnet_image_4x = sr4x_model(controlnet_image_in).clamp(0,1)

            controlnet_image = F.interpolate(controlnet_image_4x, size=pixel_values.shape[-2:], mode='bicubic')
