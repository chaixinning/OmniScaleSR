...
def realesrgan_degradation(batch,  args_degradation, use_usm=True, sf=1, resize_lq=True):
...
with torch.no_grad():
    ...
    for epoch in range(epochs):
        for num_batch, batch in enumerate(train_dataloader):
            ...
            # scale = random.uniform(1.0, 16.0)  # 均匀采样 scale
            # as_lr_size = int(512 / scale)     # 再反推 LR size
            # lr_batch = F.interpolate(lr_batch, size=(as_lr_size, as_lr_size), mode='bicubic',)
